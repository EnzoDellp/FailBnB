...
const getMisReservas = async (req, res, next) => {
  const id_usuario = req.usuario.id;
  db.query(
    "SELECT id_usuario, id_propiedad, fecha_ingreso, fecha_egreso, cantidad_viajeros FROM reservas JOIN propiedades ON reservas.id_propiedad=propiedades.id WHERE reservas.id_usuario=?",
    [id_usuario],
    (err, results) => {
      if (err) return next(err);
